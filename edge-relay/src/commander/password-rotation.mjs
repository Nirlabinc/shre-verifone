...
async function checkSitePassword(db, site, vault, publish) {
  ...
  if (daysRemaining <= 0) {
    ...
    publish?.('verifone.password.expired', 'critical', {
      siteId: site.site_id, siteName: site.site_name, daysRemaining: 0,
    });
    return;
  }
  ...
  if (passwordAgeDays >= AUTO_ROTATE_AT_DAYS && !site.password_auto_rotated_at) {
    ...
    if (result.success) {
      ...
      publish?.('verifone.password.rotated', 'info', {
        siteId: site.site_id, siteName: site.site_name, daysRemaining,
      });
      return;
    }
    ...
    if (daysRemaining <= threshold) {
      ...
      if (hoursSinceNotified >= 24) {
        publish?.('verifone.password.expiring', severity, {
          siteId: site.site_id, siteName: site.site_name,
          daysRemaining, threshold,
          autoRotateFailed: (site.password_rotation_failures || 0) > 0,
        });

        db.prepare(`UPDATE site_config SET password_user_notified_at = datetime('now') WHERE site_id = ?`)
          .run(site.site_id);
...
async function attemptAutoRotation(db, site, vault) {
  try {
    ...
    if (!changeRes.ok) return { success: false, error: `Password change failed: HTTP ${changeRes.status}` };
...
export function getPasswordHealth(db) {
  ...
  return sites.map(s => {
    ...
    return {
      ...
      passwordSetAt: s.password_set_at, passwordExpiresAt: s.password_expires_at,
      daysRemaining, passwordAgeDays,
