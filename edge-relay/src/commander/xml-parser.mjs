/**
 * Commander XML/HTML Report Parser (Edge Relay)
 *
 * Ported from shre-verifone/src/commander/xml-parser.mjs.
 * Parses Commander HTML tables and XML fragments to JSON.
 */
...
export function parseHtmlTable(html) {
  ...
  if (!tableMatch) return parseXmlReport(html);
...
export function parseXmlReport(xml) {
  ...
  try {
    ...
    if (keys.length === 1 && Array.isArray(root[keys[0]])) return root[keys[0]];
    if (typeof root === 'object' && root !== null && keys.length > 0) return [root];
    return [];
  } catch {
    return [];
  }
...
/**
 * Parse summary report (label/value pairs).
 */
...
function parseValue(val) {
  ...
  if (/^\(\$[\d,]+\.?\d*\)$/.test(s)) return -parseFloat(s.replace(/[$(),]/g, ''));
  if (/^-\$[\d,]+\.?\d*$/.test(s)) return -parseFloat(s.replace(/[$,\-]/g, ''));
  if (/^[\d.]+%$/.test(s)) return parseFloat(s) / 100;
  if (/^-?[\d,]+\.?\d*$/.test(s)) return parseFloat(s.replace(/,/g, ''));
